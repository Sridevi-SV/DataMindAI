...
from ai.llm_manager import health_check, set_privacy_mode, get_privacy_mode, generate_response
...
if datasets:
    ...
    # Reset conversation memory if scope changed
    prev_ds_id = st.session_state.get("selected_dataset_id")
    if prev_ds_id != selected_ds_id:
        from ai.agent_loop import memory
        memory.messages.clear()
        st.session_state["chat_history"] = []
        
        # Trigger rebuild of embeddings for the newly selected dataset
        try:
            requests.post(f"{BACKEND_URL}/rebuild-embeddings", json={"dataset_id": selected_ds_id}, timeout=15)
        except Exception as e:
            st.warning(f"Note: Vector store indexing pending model connectivity ({str(e)}).")
        
    st.session_state["selected_dataset_id"] = selected_ds_id
    st.session_state["selected_dataset"] = selected_ds_id
else:
...
if selected_page == "📊 Dashboard":
    ...
    active_dataset = st.session_state.get("selected_dataset")
    ds_id = active_dataset
    if not ds_id:
        ...
        st.markdown("<br/><h4>AI Orchestration & Routing Status</h4>", unsafe_allow_html=True)
        from backend.database import get_last_failover_event
        from ai.llm_manager import health_check
        
        h_status = health_check()
        last_failover = get_last_failover_event()
        
        gemini_on = h_status["gemini"]["status"] == "online"
        groq_on = h_status["groq"]["status"] == "online"
        
        # Calculate Active Provider
        if gemini_on and not h_status["privacy_mode"]:
            active_provider = "Gemini (Primary)"
        elif groq_on:
            active_provider = "Groq (Fallback)"
        else:
            active_provider = "Metadata Rule Engine (Offline)"
            
        c_status1, c_status2, c_status3, c_status4 = st.columns(4)
        with c_status1:
            g_badge = "🟢 ONLINE" if gemini_on else "🔴 OFFLINE"
            render_premium_card("Gemini Status", g_badge, "Primary AI Model", "🧠")
        with c_status2:
            gr_badge = "🟢 ONLINE" if groq_on else "🔴 OFFLINE"
            render_premium_card("Groq Status", gr_badge, "Fallback AI Model", "⚡")
        with c_status3:
            render_premium_card("Active Provider", active_provider, "Currently routing requests to", "🤖")
        with c_status4:
            if last_failover:
                timestamp = last_failover["timestamp"].split("T")[1][:8]
                event_desc = f"{last_failover['primary_model']} -> {last_failover['fallback_model']}"
                tooltip_msg = f"Logged at {timestamp}: {last_failover['error_message'][:30]}"
            else:
                event_desc = "None"
                tooltip_msg = "No failover events logged"
            render_premium_card("Last Failover Event", event_desc, tooltip_msg, "⚠️")
                ...
                if chat["role"] == "assistant" and "metadata" in chat:
                    ...
                    st.markdown(
                        ...
                        <div style="background-color: #F8FAFC; border: 1px solid #E2E8F0; padding: 0.8rem; border-radius: 8px; font-size: 0.85rem; margin-top: 0.5rem;">
                            <span class="ai-meta-badge ai-badge-model">Provider Used: {meta.get('provider', 'Gemini')}</span>
                            ...
                            <br/><span style="color: #64748B;">Response Time: {meta.get('latency_sec', 0.0):.2f} sec</span>
        ...
        if user_query:
            ...
            with st.chat_message("assistant"):
                ...
                st.markdown(
                    f"""
                    <div style="background-color: #F8FAFC; border: 1px solid #E2E8F0; padding: 0.8rem; border-radius: 8px; font-size: 0.85rem; margin-top: 0.5rem;">
                        <span class="ai-meta-badge ai-badge-model">Provider Used: {getattr(exec_state, 'provider_used', 'Gemini')}</span>
                        ...
                        <br/><span style="color: #64748B;">Response Time: {getattr(exec_state, 'llm_latency_sec', 0.0):.2f} sec</span>
            ...
            st.session_state["chat_history"].append({
                ...
                "metadata": {
                    ...
                    "provider": getattr(exec_state, 'provider_used', 'Gemini'),
                    "latency_sec": getattr(exec_state, 'llm_latency_sec', 0.0)
                }
            })
                    ...
                    if st.button("Execute SQL Query"):
                        ...
                        if "error" in q_out:
                            ...
                            with st.spinner("Formulating result explanation..."):
                                explanation, _, _, _, _ = generate_response(explain_prompt, system_instruction="Response short, direct.")
...
# ----------------- PAGE 8: SECURITY CENTER -----------------
    ...
    if priv_enabled:
        st.warning("⚠️ **Enterprise Privacy Mode is enabled.** Gemini calls will be bypassed to route all fallback requests to Groq (llama-3.3-70b-versatile). If Groq is also offline, metadata rules will apply.")
    ...
    with c1:
        ...
        st.markdown("Model Scope: `gemini-2.5-flash`")
        
    with c2:
        st.markdown("##### Fallback Routing (Groq)")
        status_color = "🟢 ONLINE" if h_state["groq"]["status"] == "online" else "🔴 OFFLINE"
        st.markdown(f"Status: **{status_color}**")
        st.markdown("Model Scope: `llama-3.3-70b-versatile`")
        ...
        if st.button("Generate & Index Data Catalog"):
            ...
            with st.spinner("Processing file through ingestion pipeline..."):
                try:
                    ...
                    if res.status_code == 200:
                        res_json = res.json()
                        new_ds_id = res_json.get("dataset_id")
                        st.cache_data.clear()
                        ...
                        st.session_state["selected_dataset_id"] = new_ds_id
                        st.session_state["selected_dataset"] = new_ds_id
