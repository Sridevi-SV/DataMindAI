...
def add_failover_event(primary_model, fallback_model, error_message, latency_ms):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
            """INSERT INTO failover_events (timestamp, primary_model, fallback_model, error_message, latency_ms)
               VALUES (?, ?, ?, ?, ?)""",
            (datetime.now().isoformat(), primary_model, fallback_model, error_message, latency_ms)
        )
        conn.commit()
    except Exception as e:
        print(f"[DATABASE] Error adding failover event: {e}")
    finally:
        conn.close()

def get_last_failover_event():
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM failover_events ORDER BY id DESC LIMIT 1")
        row = cursor.fetchone()
        return dict(row) if row else None
    except Exception as e:
        print(f"[DATABASE] Error fetching last failover event: {e}")
        return None
    finally:
        conn.close()
