        ...
        def generate_sql(question, dataset_id, **kwargs):
            ...
            try:
                ...
                for t in tables:
                    ...
                    if not t_id:
                        continue
                    cursor.execute("SELECT name, data_type, description, is_pii FROM columns WHERE table_id = ?", (t_id,))
                    columns = [dict(row) for row in cursor.fetchall()]
                    t["columns"] = columns
                    tables_meta.append(t)
                    
                    catalog_schema[t_name] = {
                        "columns": [c.get("name") or c.get("column_name") for c in columns if c.get("name") or c.get("column_name")],
                        "is_pii": [c.get("name") or c.get("column_name") for c in columns if (c.get("name") or c.get("column_name")) and c.get("is_pii", 0) == 1]
                    }
            finally:
                conn.close()
                
            schema_prompt = generate_schema_context_prompt(tables_meta)
            if isinstance(schema_prompt, dict) and not schema_prompt.get("success", True):
            ...
            sql_query, _, model_name, _, _ = generate_response(prompt, system_instruction="You output ONLY standard SQL code. No explanation.")
            ...
            if model_name == "ERROR":
                ...
                return {
                    "generated_sql": "",
                    "is_valid": False,
                    "validation_error": f"AI service is currently unavailable. Details: {clean_error_message(sql_query)}"
                }
