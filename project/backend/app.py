...
from ai.vector_store import index_catalog, rebuild_vector_store
...
async def upload_file(
    ...
    try:
        ...
        for table_meta in metadata_list:
            ...
            """
            table_desc, _, _, _, _ = generate_response(desc_prompt, system_instruction="Response short, direct.")
            ...
            for col in cols:
                ...
                """
                col_desc, _, _, _, _ = generate_response(col_desc_prompt, system_instruction="Response short, direct.")
...
def connect_postgres(payload: ConnectionPayload):
    ...
    try:
        ...
        for table_meta in metadata_list:
            ...
            table_desc, _, _, _, _ = generate_response(desc_prompt, "You describe tables short.")
            ...
            for col in cols:
                ...
                col_desc, _, _, _, _ = generate_response(col_desc_prompt, "Explain column names.")
...
def rebuild_embeddings_endpoint(payload: dict):
    dataset_id = payload.get("dataset_id")
    if not dataset_id:
        raise HTTPException(status_code=400, detail="dataset_id is required.")
    try:
        rebuild_vector_store(dataset_id)
        return {"status": "success", "message": f"Embeddings rebuilt for dataset {dataset_id}"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
