...
def test_agent_loop_routing_and_planning(mock_generate_response):
    ...
    mock_generate_response.side_effect = [
        ...
        ("QUALITY", "Gemini", "gemini-1.5-flash", "Classified as data quality", 0.1),
        # Call 2: Final response reasoning
        ("Mock data quality answer showing schema health metrics.", "Gemini", "gemini-1.5-flash", "Standard response", 0.5)
    ]
...
def test_agent_loop_pii_security_compliance(mock_generate_response):
    ...
    mock_generate_response.side_effect = [
        # Call 1: Intent detection
        ("SQL", "Gemini", "gemini-1.5-flash", "Classified as SQL", 0.1),
        # Call 2: Final response reasoning containing actual raw emails (which triggers rejection)
        ("Here is the requested email list: Alice (alice@gmail.com), Bob (bob@gmail.com)", "Gemini", "gemini-1.5-flash", "Standard response", 0.5)
    ]
