...
def generate_response(prompt: str, system_instruction: str = None) -> tuple[str, str, str]:
    """
    Generates a response from the appropriate LLM.
    Handles Privacy Mode, local failover, and PII scrubbing.
    Returns: (response_text, model_used, routing_reason)
    """
    # Scrub prompt for PII before dispatch
    secured_prompt = scrub_sensitive_patterns(prompt)
    
    # 1. Privacy Mode check
    if PRIVACY_MODE:
        reason = "Enterprise Privacy Mode active. Strictly using Ollama."
        try:
            res = call_ollama_api(secured_prompt, system_instruction)
            return scrub_sensitive_patterns(res), OLLAMA_MODEL, reason
        except Exception as e:
            return f"Ollama local execution failed: {str(e)}", "N/A", f"Privacy Mode Error: {reason}"
            
    # 2. Try Gemini (Primary Model)
    try:
        reason = "Primary Model Gemini API called."
        res = call_gemini_api(secured_prompt, system_instruction)
        return scrub_sensitive_patterns(res), GEMINI_MODEL, reason
    except Exception as gemini_err:
        # Catch rate limit, network, timeout, quota errors, etc.
        # Fallback to local Ollama
        reason = f"Gemini failed ({type(gemini_err).__name__}). Automatic Failover to Ollama."
        try:
            res = call_ollama_api(secured_prompt, system_instruction)
            return scrub_sensitive_patterns(res), f"{OLLAMA_MODEL} (Fallback)", reason
        except Exception as ollama_err:
            return (
                f"Both cloud and local AI modules failed to respond.\n"
                f"Gemini error: {str(gemini_err)}\n"
                f"Ollama error: {str(ollama_err)}", 
                "N/A", 
                "Double Failover Failure"
            )
